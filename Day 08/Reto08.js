...
function organizeGifts(gifts) {
  ...
  gifts.match(/\d+[a-zA-Z]/g).forEach(entry => {
    const count = +entry.match(/\d+/)[0];
    const type = entry.match(/[a-zA-Z]/)[0];
    const pallets = count / 50 | 0;
    const boxes = count / 10 % 5 | 0;
    const remainingGiftsInBox = count % 10;

    result[type] = (result[type] || '') +
      `[${type}]`.repeat(pallets) +
      `{${type}}`.repeat(boxes) +
      (remainingGiftsInBox > 0 ? `(${type.repeat(remainingGiftsInBox)})` : '');
  });
